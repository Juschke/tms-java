...
public interface QuoteRepository extends JpaRepository<Quote, UUID> {
        @org.springframework.data.jpa.repository.Query("SELECT q FROM Quote q LEFT JOIN FETCH q.customer WHERE q.tenant.id = :tenantId AND q.deletedAt IS NULL")
        List<Quote> findByTenantIdAndDeletedAtIsNull(
        ...
        @org.springframework.data.jpa.repository.Query(value = "SELECT q FROM Quote q " +
                        ...
                        "AND (:maxAmount IS NULL OR q.grossAmount <= :maxAmount)", countQuery = "SELECT count(q) FROM Quote q "
                                        +
