...
public interface PartnerRepository extends JpaRepository<Partner, UUID> {
        ...
        @org.springframework.data.jpa.repository.Query("SELECT DISTINCT p FROM Partner p JOIN p.languagePairs lp " +
                        "WHERE p.tenant.id = :tenantId AND p.deletedAt IS NULL " +
                        "AND lp.sourceLanguage.id = :sourceLangId AND lp.targetLanguage.id = :targetLangId")
        List<Partner> findByLanguagePair(
                        @org.springframework.data.repository.query.Param("tenantId") UUID tenantId,
                        @org.springframework.data.repository.query.Param("sourceLangId") UUID sourceLangId,
                        @org.springframework.data.repository.query.Param("targetLangId") UUID targetLangId);
